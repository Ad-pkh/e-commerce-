...
class categoryservice {
  ...
  categoryUpdate = async (id, data) => {
    ...
    try {
      ...
      const update = await categorymodel.findByIdAndUpdate(
        id,
        { $set: { title: data.title, status: data.status, brand: data.brand } },
        { new: true }
      ); //without 3rrd arg it returns data before update and with 3rd arg it returns data after update
      if (!update) {
        //to do //update brand id not brand name
  ...
  brandfilter = async (brand) => {
    try {
      ...
      const brand_id = await brandmodel.find(
        { title: { $in: brandsArray } },
        "_id"
      ); // Only get IDs
      //console.log(brands);
      ...
      if (!brand_id.length) {
        throw {
          status: 404,
          message: "Brand not found. Please register the brand first.",
        };
      }
  ...
  rootfilter = async (parent) => {
    try{
     const data = await categorymodel.findOne({ title: parent }, "_id");
